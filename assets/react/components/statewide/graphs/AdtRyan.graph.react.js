...
var GraphContainer = React.createClass({
    ...
    componentDidMount:function(){
            this._loadData(this.props.classByMonth);
    },

    componentWillReceiveProps:function(nextProps){
            this._loadData(nextProps.classByMonth);
    },
    _loadData:function(rawData){
        var scope = this;

        //Returns filtered and sorted data

        //For now, takes in classByMonth, which is passed to this component as a property 

        //For now, simply processing client side data here instead of in both the
        //chart and graph functions

        if(Object.keys(rawData.getDimensions()).length > 0){

            var stationADT= rawData.getGroups()
            ...
            var output = stationADT.map(function(d){
